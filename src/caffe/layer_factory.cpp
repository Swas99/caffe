...
shared_ptr<Layer<Dtype> > Get2x2WinogradTransLayer(
  ...
  if (engine == ConvolutionParameter_Engine_CAFFE) {
    return shared_ptr<Layer<Dtype> >(new Winograd2x2TransLayer<Dtype>(param));
#ifdef USE_CUDNN
...
REGISTER_LAYER_CREATOR(Winograd2x2Trans, Get2x2WinogradTransLayer);


//Get Winograd Conv layer according to engine.
template <typename Dtype>
shared_ptr<Layer<Dtype> > Get2x2WinogradConvLayer(
    const LayerParameter& param) {
   ConvolutionParameter conv_param = param.convolution_param();
  ConvolutionParameter_Engine engine = conv_param.engine();
#ifdef USE_CUDNN
  ...
  if (engine == ConvolutionParameter_Engine_CAFFE) {
    return shared_ptr<Layer<Dtype> >(new Winograd2x2ConvLayer<Dtype>(param));
#ifdef USE_CUDNN
  } else if (engine == ConvolutionParameter_Engine_CUDNN) {
    if (use_dilation) {
      LOG(FATAL) << "CuDNN doesn't support the dilated convolution at Layer "
...
REGISTER_LAYER_CREATOR(Winograd2x2Conv, Get2x2WinogradConvLayer);
